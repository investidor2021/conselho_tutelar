...
const btnCalcular = document.getElementById('btnCalcular');// --- START OF FILE script.js ---
...
const groupFeriasNormaisContainer = document.getElementById('groupFeriasNormaisContainer'); // Note: This ID is not in the HTML
const chkRescisao = document.getElementById('isRescisao');
const infoRescisaoDiv = document.getElementById('infoRescisao');
const groupRescisaoContainer = document.getElementById('groupRescisaoContainer'); // Note: This ID is not in the HTML
...
const groupDataInicioFeriasDiv = document.getElementById('groupDataInicioFerias'); // Note: This ID is not in the HTML
const fldDataInicioFerias = document.getElementById('dataInicioFerias'); // Note: This ID is not in the HTML
const fldDataFimFerias = document.getElementById('dataFimFerias'); // Note: This ID is not in the HTML
...
function calcularDataFimFerias() {
    // This function depends on fldDataInicioFerias and fldDataFimFerias which are not in the HTML
...
function executarCalculo() {
   ...
   if (isRescisaoChecked) {
        ...
        // This logic requires fldDataInicioFerias, which is missing from the HTML.
        // The code will throw an error here if a user checks the "Férias" box.
        // For now, leaving the logic as is, but it's a potential future bug.
        ...
        if ( (mesInicioFerias === mesFimFerias && diaFimFerias < diasNoMesFimFerias && diaInicioFerias > 1) ||
             (mesInicioFerias === mesFimFerias && diaFimFerias < diasNoMesFimFerias && diaInicioFerias === 1) ||
             (mesInicioFerias !== mesFimFerias && diaFimFerias < diasNoMesFimFerias)
           ) {
            diasTrabalhadosMesTermino = diasNoMesFimFerias - diaFimFerias;
            const valorDiaMesFim = salarioBaseInput / diasNoMesFimFerias;
        ...
        if (saldoSalarioMesTermino > 0) {
            ...
            if (pag.proventos.saldoSalario > 0) {
            ...
            htmlInformativoProximoMes += `<div class="resumo-item" style="font-weight:bold; margin-top: 5px;"><span>TOTAL DESCONTOS:</span> <span>${formatCurrency(pag.totais.descontos)}</span></div><br>`;
            htmlInformativoProximoMes += `<div class="resumo-item total"><span>LÍQUIDO A RECEBER:</span><span>${formatCurrency(pag.totais.liquido)}</span></div>`;
        } else if (!calculoAtual.pagamentoSaldoMesInicioFerias && htmlInformativoProximoMes === '') {
             htmlInformativoProximoMes = `<hr style="margin: 15px 0 10px;">
...
function gerarPDF() {
    try {
        ...
        doc.setFontSize(13); doc.setFont(undefined, 'bold'); doc.setTextColor(3, 86, 179);
        ...
        doc.setFont(undefined, 'normal'); doc.text(calculoAtual.nome, margemEsquerda + 12, linhaY);
        
        const refPrincipalFormatada = calculoAtual.referenciaPagamento.split('-').reverse().join('/');
        const textoRef = "Ref. Pag. Principal:";
        const larguraTextoRef = doc.getTextWidth(textoRef) + 2;
        const xRefLabel = margemDireita - doc.getTextWidth(refPrincipalFormatada) - larguraTextoRef - 10;
        ...
        if (calculoAtual.isFeriasNormais) {
            ...
            doc.setFont(undefined, 'normal'); doc.text(`RECIBO DE FÉRIAS E SALÁRIOS`, margemEsquerda + 10, linhaY);
            linhaY += espacamentoItemPDF;
            doc.setFont(undefined, 'bold'); doc.text("Período de Gozo Férias:", margemEsquerda, linhaY);
            doc.setFont(undefined, 'normal');
            doc.text(`${calculoAtual.dataInicioFerias.split('-').reverse().join('/')} a ${calculoAtual.dataFimFerias}`, margemEsquerda + 38, linhaY);
            linhaY += espacamentoItemPDF;
        } else if (!calculoAtual.isRescisao) {
            doc.setFont(undefined, 'bold'); doc.text("Tipo:", margemEsquerda, linhaY);
            doc.setFont(undefined, 'normal'); doc.text(`PAGAMENTO MENSAL (Contr. Individual)`, margemEsquerda + 10, linhaY);
            linhaY += espacamentoItemPDF;
        }
        ...
        const desenharSecaoPagamentoPDF = (pagamentoObj, tituloSecao, yAtual, isEstimativa = false) => {
            ...
            for (const key in pagamentoObj.proventos) {
                ...
                let desc = key.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase());
            ...
            if (pagamentoObj.descontos.irrf > 0 && pagamentoObj.resultadoIRRF) {
        ...
        if (linhaY + 20 > doc.internal.pageSize.getHeight() - 15) { doc.addPage(); linhaY = 30; }
 ...
 chkFeriasNormais.addEventListener('change', function() {
    if (this.checked) {
        ...
        if (groupFaltasContainer) {
         ...
         if(groupDiasFeriasDiv) groupDiasFeriasDiv.style.display = 'none';
         if(groupDataInicioFeriasDiv) groupDataInicioFeriasDiv.style.display = 'none';
         if(fldDataInicioFerias) fldDataInicioFerias.value = '';
         if(fldDataFimFerias) fldDataFimFerias.textContent = '';
         labelSalarioBase.textContent = 'Salário Base Mensal (R$):';
         if (groupFaltasContainer) {
            const labelFaltas = groupFaltasContainer.querySelector('label[for="faltas"]');
...
window.addEventListener('DOMContentLoaded', () => {
    if (fldTetoInss.value) {
        ...
        fldTetoInss.value = formatToBRL(8157.41);
        fldTetoInss.classList.add('formatted');
    }
